...
export async function generateStaticParams() {
    const data: PokemonResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=251`)
        .then(res => res.json())

    const static251Pokemons = data.results.map(pokemon => ({
        name: pokemon.name
    }))

    return static251Pokemons.map(name => {
        name: name
    })
}
